...
class Adam_meta(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), meta = {}, eps=1e-8, weight_decay=0, amsgrad=False):
        if not 0.0 <= lr:
                    ...
                    with torch.no_grad():
                        ...
                        # diffusion entre les bk dans les deux sens + metaplasticité sur le dernier
                            if bk_idx==(n_bk-1):
                                ...
                                state['bk'+str(bk_idx)+'_t'] = torch.where(condition, state['bk'+str(bk_idx)+'_t-1'] + (ratios[bk_idx-1]/areas[bk_idx])*decayed_m*(state['bk'+str(bk_idx-1)+'_t-1'] - state['bk'+str(bk_idx)+'_t-1']) + (ratios[bk_idx]/areas[bk_idx])*(state['bk'+str(bk_idx+1)+'_t-1'] - state['bk'+str(bk_idx)+'_t-1']), state['bk'+str(bk_idx)+'_t-1'] + (ratios[bk_idx-1]/areas[bk_idx])*(state['bk'+str(bk_idx-1)+'_t-1'] - state['bk'+str(bk_idx)+'_t-1']) + (ratios[bk_idx]/areas[bk_idx])*(state['bk'+str(bk_idx+1)+'_t-1'] - state['bk'+str(bk_idx)+'_t-1']))
                            else:
