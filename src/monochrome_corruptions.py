...
def fog(x, severity=1):
    ...
    max_val = x.max()
    x = x + c[0] * plasma_fractal(wibbledecay=c[1])[:x.shape[0], :x.shape[1]]
...
def motion_blur(x, severity=1):
    ...
    x = PILImage.fromarray(x.squeeze()*255).convert('RGB')
    output = BytesIO()
...
def frost(x, severity=1):
    ...
    x = np.clip(c[0] * x[..., np.newaxis] + c[1] * frost_img / 255, 0, 1)
    return sk.color.rgb2gray(x.squeeze())

def jpeg_compression(x, severity=1):
    c = [18, 15, 12, 10, 7][severity - 1]
    x = PILImage.fromarray(x*255).convert('RGB')
    output = BytesIO()
